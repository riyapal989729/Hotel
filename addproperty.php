<?php
    ...
    if(isset($_POST['add'])){
        ...
        $status=$_POST['status'];
        $data="insert into property(addproperty,status)values('$addproperty','$status')";
        $run=mysqli_query($conn,$data);
        $_SESSION['error']="Add User successfully";
        header('location:allpropertydata.php');
        exit();
    ...
    if(isset($_GET['id'])){
        $check=1;
        $id=$_GET['id'];
        $update="select * from property where id= '$id'";
    ...
    if(isset($_POST['update'])){
        $addproperty= $_POST['addproperty'];
        $status=$_POST['status'];
        $sql="update property set addproperty='$addproperty',status ='$status'  where id='$id'";
                        ...
                        <div class="container">
                            <form method="POST">
                                <div class="form-group mt-4">
                                    ...
                                    <input type="text" class="form-control" id="addproperty" placeholder="Enter your property" name="addproperty" value="<?php echo $addproperty; ?>" />
                                </div>
