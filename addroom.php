<?php
    ...
    if(isset($_POST['add'])){
        ...
        $status=$_POST['status'];
        $data="insert into roomtype(addroom,status)values('$addroom','$status')";
        $run=mysqli_query($conn,$data);
        $_SESSION['error']="Add User successfully";
        header('location:roomdatatype.php');
        exit();
    ...
    if(isset($_POST['update'])){
        $addroom= $_POST['addroom'];
        $status=$_POST['status'];
        $sql="update roomtype set addroom='$addroom',status ='$status'  where id='$id'";
